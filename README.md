...
- [x] Explain the difference between `Relational Databases` and `SQL`.
  ...
  Relational databases are a type of database where tables are related to each other. SQL is a language used to query databases.

- [x] Why do tables need a `primary key`?

  Tables have columns and rows, primary keys give unity to a collection of columns by forming a unique row.
